    ...
    <div className="border border-rule p-4">
      ...
      <ul className="mt-3 space-y-1 text-sm">
        {side.players.map((player) => (
          <li key={`${player.slot}-${player.name}`} className="flex justify-between gap-3">
            <span>
              <span className="text-[11px] uppercase tracking-wider text-ink/40">{player.slot || player.position}</span>{" "}
              {player.name}
            </span>
      ...
      {open ? (
        championship.rosters?.winner && championship.rosters.loser ? (
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            ...
            <SideRoster
              label="Runner-up"
              ownerName={championship.runnerUpName}
              side={championship.rosters.loser}
            />
          </div>
        ) : (
